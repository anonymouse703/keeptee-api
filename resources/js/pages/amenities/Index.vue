...
const filteredAmenities = computed(() => {
    if (!searchFilterRef.value) return amenitiesData.value

    const filters = searchFilterRef.value.filters
    let result = [...amenitiesData.value]
    ...
    if (filters.sort) {
        switch (filters.sort) {
            ...
            case 'name_desc':
                ...
                break
            case 'count_desc':
                result.sort((a, b) => b.amenities_count - a.amenities_count)
                break
            case 'count_asc':
                result.sort((a, b) => a.amenities_count - b.amenities_count)
                break
            case 'created_desc':
                result.sort(
                    (a, b) =>
...
const sortOptions = [
    ...
    { label: 'Most Amenities', value: 'count_desc' },
    { label: 'Least Amenities', value: 'count_asc' },
...
<template>
    <AppLayout>
        <div class="relative space-y-6 p-4 md:p-6 lg:p-8">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400">
                                Manage and categorize your property amenities
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <Badge variant="outline" class="text-xs">
                            {{paginationLinks.total }} amenities total
                        </Badge>
                        <Badge
                            variant="outline"
                            class="text-xs bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300"
                        >
                            {{ filteredAmenities.length }} displayed
                        </Badge>
                    </div>
                </div>

                <div class="shrink-0">
                    <BaseButton @click="router.visit('/amenities/create')">
                        <Home class="size-4" />
                        Add New Amenity
                    </BaseButton>
                </div>
            ...
            <!-- Data Table -->
            <DataTable :columns="columns" :data="filteredAmenities">
